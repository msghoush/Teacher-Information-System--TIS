...
class SaaSPostActivationStateTests(unittest.TestCase):
    ...
    def test_active_tenant_outranks_stale_checkout_state_on_account_setup(self):
        ...
        self.assertIn("Organization Account", response.text)
        self.assertIn("Confirmed Active Academy", response.text)
        self.assertIn("Enter TIS Platform", response.text)
        self.assertNotIn("Continue to Secure Payment", response.text)
        self.assertNotIn('aria-label="TIS Account guided setup"', response.text)
        self.assertNotIn('aria-label="Onboarding progress"', response.text)
        self.assertNotIn('class="account-overview-actions"', response.text)
        db = self.Session()
        try:
            ...
            self.assertEqual(account.onboarding_status, "checkout_ready")
    ...
    def test_active_workspace_remains_available_when_subscription_link_needs_review(self):
        ...
        self.assertIn("Enter TIS Platform", response.text)
        self.assertIn("Organization Account", response.text)
        self.assertIn("Review subscription status and billing recovery options.", response.text)
        self.assertNotIn("Continue to Secure Payment", response.text)
        self.assertNotIn('aria-label="TIS Account guided setup"', response.text)
        self.assertNotIn('aria-label="Onboarding progress"', response.text)
