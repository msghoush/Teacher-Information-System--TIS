...
(() => {
  ...
  // Shared "New Program" creation (used by this module's Program list and by the
  // System Configuration > Talent & Potential shell, so create validation and the
  // two canonical API calls exist exactly once). The server still authorizes both.
  const newProgramFormFields = planningGrades => field('name','Program name','', 'text',true)+area('description','Program description')+(planningGrades.length?`<fieldset><legend>Eligible Grades</legend>${planningGrades.map(g=>check(`grade_${g}`,g==='KG'?'KG':`Grade ${g}`,false)).join('')}</fieldset>`:'<p class="tp-inline-empty">No Grades are configured in Planning for this Academic Year.</p>');
  const newProgramFormHtml = planningGrades => form('create-program','New Program',newProgramFormFields(planningGrades),'Save Program');
  async function submitNewProgram(formEl, {api, year, planningGrades}) {
    const d=new FormData(formEl);
    const grades=planningGrades.filter(g=>d.has(`grade_${g}`));
    if(!grades.length) throw Object.assign(new Error('Choose at least one eligible Grade.'),{validation:true});
  ...
  async function render(ctx, options = {}) {
    ...
    if (viaHash && pid && bundleCache && bundleCache.key === bundleKey) {
      ...
      if(!pid) {
        ...
        const rows=summaries.map(({program,current,type})=>`<article class="tp-program-card" data-program-row data-search="${esc(program.name.toLowerCase())}"><h3>${logoBadge(program,'tp-logo-sm')} ${esc(program.name)}</h3><p>${esc(program.description||'Configure the assessment framework and planned evaluations for this Program.')}</p><dl><dt>Eligible Grades</dt><dd>${current?.eligible_grade_levels?.length?`<ul class="tp-chip-list">${current.eligible_grade_levels.map(g=>`<li>${g==='KG'?'KG':`Grade ${esc(g)}`}</li>`).join('')}</ul>`:'Not set'}</dd><dt>Scoring Mode</dt><dd>${esc(type)}</dd></dl><div class="tp-row-actions"><a class="tp-primary-link" href="${esc(href('programs',{program_id:program.id}))}">${icon('eye')}Open Program</a>${can('talent_analytics.view')?`<a href="${esc(href('analytics',{program_id:program.id}))}">${icon('eye')}View Results</a>`:''}${(program.actions||[]).includes('delete')?button('delete-program','Delete',`data-id="${program.id}"`,'trash'):''}</div></article>`).join('');
        root.innerHTML=`<div data-status role="status" aria-live="polite"></div><div class="tp-section-lede"><div><h2>Programs</h2><p>Create the Program and align it with eligible Grades. Configure the rubric separately from the Program row.</p></div>${manage?'<button type="button" data-action="new-program">New Program</button>':''}</div><label class="tp-search">Search Programs<input type="search" data-program-search placeholder="Search by Program name"></label><div class="tp-program-cards">${rows||'<p class="tp-empty">No Programs yet.</p>'}</div>${manage?`<div data-new-program hidden>${newProgramFormHtml(planningGrades)}</div>`:''}`;
        ...
        root.querySelector('form')?.addEventListener('submit',async event=>{
          ...
          try{
            await submitNewProgram(formEl,{api,year,planningGrades});
    ...
    // ADR 0039: readiness is exactly Competency + KPI + Level, saved - not
    // every achievement description filled in and not an Evaluation Period
    // already scheduled. Those remain useful but are no longer a normal-user
    // Ready/Draft gate.
    const rubricReady=Boolean(members.length&&members.some(m=>rubricForCompetency(m.id)?.levels?.length || levels.length));
    ...
    if(!explicitSetup){
      ...
      const scoringMode=kpi?.enabled?'Numeric result + KPI':(rubricReady?'KPI':'Not set');
      ...
      root.innerHTML=`<div data-status role="status" aria-live="polite"></div>${ctx.configuration?'':`<a href="${esc(href('programs',{program_id:''}))}">← All Programs</a>`}<header id="tp-overview" class="tp-section-lede">${logoBadge(program,'tp-logo-md')}<div><h2>${esc(program.name)}</h2><p>${esc(program.description||'')}</p><p>${esc(yearLabel)}</p></div></header><section class="tp-program-summary" aria-label="Program summary"><div class="tp-readiness-banner ${setupComplete?'is-ready':'is-incomplete'}"><strong>${setupComplete?'✓ Ready':'Needs setup'}</strong><span>${setupComplete?'Competency, KPI, and Level are configured. Start Assessment is available.':(missingRequirement||'Complete the Program setup below.')}</span></div><dl><div><dt>Grades</dt><dd>${grades||'Not configured'}</dd></div><div><dt>Competencies</dt><dd>${members.length}</dd></div><div><dt>KPI</dt><dd>${rubrics.filter(r=>r.framework_competency_id!=null).length || (levels.length?1:0)}</dd></div><div><dt>Scoring Mode</dt><dd>${esc(scoringMode)}${kpi?.enabled?` <span class="tp-badge">Numeric result enabled</span>`:''}</dd></div><div><dt>Evaluation Periods</dt><dd>${periodCount}</dd></div></dl></section>${gradeRubricSummary?`<section><h3>KPI Overview</h3><div class="tp-grade-accordion">${gradeRubricSummary}</div></section>`:''}<div class="tp-summary-actions">${manage?`<a href="#tp-basics">${icon('edit')}Edit Program</a><a href="#tp-rubric">${icon('edit')}Build / Edit Competency, KPI &amp; Level</a>`:''}${can('talent_evaluation_plans.view')?`<a href="#tp-schedule">${can('talent_evaluation_plans.manage')?'Manage Evaluation Plan':'View Evaluation Plan'}</a>`:''}${can('talent_assessments.view')?`<a href="${esc(href('assessments'))}">${icon('eye')}Open Assessments</a>`:''}${can('talent_analytics.view')?`<a href="${esc(href('portfolio'))}">${icon('eye')}View Results</a>`:''}${govern&&setupComplete&&(program.status==='draft'||framework?.status==='draft')?button('finish-setup','Finish setup'):''}</div>`;
      root.onclick=async event=>{if(busy)return;const b=event.target.closest('[data-action]');if(b?.dataset.action==='finish-setup')await finishSetup();};
      return;
    }
    const stepReason={assess:assessRemaining?`${assessRemaining} item${assessRemaining===1?'':'s'} remaining`:''};
    const nav=(ctx.configuration||activeStep==='basics'||activeStep==='rubric')?'':[['assess','What we assess'],['schedule','Evaluation Plan']].map(([key,label],index)=>`<a href="${hashes[key]}" data-step="${key}" class="tp-step ${key===activeStep?'tp-step-current':stepState[key]?'tp-step-complete':'tp-step-pending'}" ${key===activeStep?'aria-current="step"':''}><span>${stepState[key]?icon('check'):index+1}</span><b>${label}</b>${stepReason[key]?`<small>${esc(stepReason[key])}</small>`:''}</a>`).join('');
    ...
    root.onclick=async event=>{
      ...
      if(a==='remove-member-versioned'){
        ...
        await api(`${base}/frameworks/${created.id}/competencies/${b.dataset.key}?expected_revision=${created.revision}`,{method:'DELETE'});
        params.set('framework_id',String(created.id));
        bundleCache=null;
        if(typeof window!=='undefined')window.location.hash='#tp-rubric';
        await fullRefresh();
      ...
      if(a==='finish-setup'){await finishSetup();return;}
  ...
  function dispose(){if(typeof window==='undefined')return;if(unloadGuard)window.removeEventListener('beforeunload',unloadGuard);if(hashGuard)window.removeEventListener('hashchange',hashGuard);}
  if(typeof module!=='undefined'&&module.exports)module.exports={render,esc,kpiComponents,numeric,logoInitials,logoBadge,newProgramFormHtml,submitNewProgram,dispose};
  if(typeof window!=='undefined'){window.TalentProgramIdentity={logoInitials,logoBadge};window.TalentProgramWorkspace={render,icon,newProgramFormHtml,submitNewProgram,dispose};}
})();
