  ...
  async function render(ctx, options = {}) {
    ...
    const rubricGradeSections=descriptorGrades.map((grade,gradeIndex)=>{
      ...
      const competencyCards=gradeMembers.map(m=>{
        ...
        return `<article class="tp-card tp-rubric-competency"><div class="tp-competency-node-head"><div><span class="tp-node-label">Competency</span><h4>${esc(memberName(m))}</h4><p>${esc(m.description||'')}</p></div>${editable&&canDeleteCompetency?button('remove-member','Delete Competency',`data-key="${m.competency_id}"`,'trash'):''}</div>${rubricBody}</article>`;
      }).join('');
    ...
    const competencyRows=members.map(m=>`<tr><td>${esc(m.grade_level?(m.grade_level==='KG'?'KG':`Grade ${m.grade_level}`):'All Grades')}</td><th scope="row">${esc(memberName(m))}</th><td>${esc(m.description||'—')}</td><td><div class="tp-row-actions">${editable?`${button('reveal-editor','Edit',`data-editor-key="member-${m.competency_id}"`,'edit')}${canDeleteCompetency?button('remove-member','Delete Competency',`data-key="${m.competency_id}"`,'trash'):''}`:''}</div><div data-editor="member-${m.competency_id}" hidden>${editable?form(`member:${m.competency_id}`,'Edit competency',select('grade_level','Grade',gradeOptions,m.grade_level||'')+field('label','Name',memberName(m),'text',true)+area('description','Description',m.description)):''}</div></td></tr>`).join('');
    ...
    const levelRows=levels.map(l=>`<tr><th scope="row">${rubricVisual().badge(l,levels)}</th><td>${esc(l.description||'—')}</td><td><div class="tp-row-actions">${editable?`${button('reveal-editor','Edit',`data-editor-key="level-${l.id}"`,'edit')}${canDeleteRubricLevel?button('remove-level','Delete Level',`data-key="${l.id}"`,'trash'):''}`:''}</div><div data-editor="level-${l.id}" hidden>${editable?form(`level:${l.id}`,'Edit level',field('label','Level name',l.label,'text',true)+area('description','Description',l.description)+field('numeric_value','Numeric value (optional)',l.numeric_value,'number')):''}</div></td></tr>`).join('');
