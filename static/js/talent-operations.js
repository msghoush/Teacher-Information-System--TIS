...
(() => {
  ...
  // Acceptance D: one full-width, readable rubric choice per level. Rank, title
  // and description stay on separate visual lines (never compressed vertically);
  // the whole tile is the radio label, so the click target is large and native
  // keyboard/focus semantics are preserved.
  const rubricOption=(cid,level,levels,selected,description)=>{
    const count=Math.max((levels||[]).length,1);
    const pos=rubricVisual&&typeof rubricVisual.position==='function'
      ? rubricVisual.position(level,levels)
      : {index:0,count};
    const strength=rubricVisual&&typeof rubricVisual.intensity==='function'
      ? rubricVisual.intensity(pos.index,pos.count)
      : 0;
    const rank=`${pos.index+1} / ${pos.count}`;
    return `<label class="tp-level-option tp-rubric-choice"><input type="radio" name="level-${cid}" value="${esc(level.id)}" ${selected?'checked':''}><span class="tp-rubric-tile${selected?' is-selected':''}" style="--tp-rubric-intensity:${strength.toFixed(3)}"><span class="tp-rubric-tile-head"><span class="tp-rubric-rank" aria-hidden="true">${rank}</span><strong>${esc(level.label)}</strong>${selected?'<span class="tp-rubric-tile-check" aria-hidden="true">✓</span>':''}</span>${description?`<span class="tp-rubric-tile-desc">${esc(description)}</span>`:''}</span></label>`;
  };
  ...
  async function render(ctx) {
    ...
    if(params.get('assessment_id')) {
      ...
      // Owner amendment 2026-09-12: current editable criteria are strictly
      // Grade-aligned. Intentionally unscoped content may apply, but content
      // explicitly assigned to another Grade is never substituted. Historical
      // read-only evidence remains inspectable on its original Framework.
      ...
      const savedCompetencyIds=new Set(results.map(row=>Number(row.framework_competency_id)));
      ...
      // Back preserves the exact Academic Year/Program/Evaluation(Cycle)
      // context this Assessment was opened from - never the Talent landing
      // page - so the Owner can return to the same Student list and pick
      // another Student.
      const backToStudentsUrl=url('assessments',{
        assessment_id:'',
