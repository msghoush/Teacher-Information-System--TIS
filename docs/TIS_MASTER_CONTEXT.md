...
- `/api/talent/assessment-cycles`: Cycle metadata, authorization-filtered Draft/frozen population, and organization-governed Open/Close/additive Open-population synchronization API (`routers/talent_assessment_cycles.py`, dedicated `talent_assessment_cycles.*` permissions). Synchronization is expected-revision protected and adds currently eligible missing members only; Cycles have no Branch ownership and identifiable population access is filtered by resolved/frozen Branch context.
