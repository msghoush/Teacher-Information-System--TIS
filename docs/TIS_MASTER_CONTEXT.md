...
## Grade-Specific Talent Rubric Descriptors And Student History Force Delete

Talent Frameworks now support an additive Grade-specific descriptor override at the exact grain `Framework Competency + Rubric Level + Grade`. The existing generic `Framework Competency + Rubric Level` descriptor remains the backward-compatible fallback. Program authoring presents descriptor cells for the Program's eligible Grades, and Student Assessment presentation resolves the exact Grade-specific descriptor from the Assessment's recorded historical Grade before falling back to the generic descriptor. This permits one competency and one ordered rubric to carry Grade-appropriate achievement language without fabricating separate Grade competencies. The new persistence table is `talent_grade_competency_rubric_descriptors`, introduced by migration `20260911_001_talent_grade_specific_rubric_descriptors`; Framework fingerprinting, cloning, Draft-only mutation, tenant scope, and audit semantics include these rows.
