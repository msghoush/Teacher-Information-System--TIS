...
subject and weekly-period requirement. It is isolated by branch and academic year,
supports retained inactive/retired rows, and database constraints prevent cross-
scope section or Subject references and duplicate active section-subject demand.
...
- `talent_assessment_cycle_service.py`: SchoolGroup-wide Assessment Cycle lifecycle, explicit-time Draft eligibility preview, atomic opening population, accepted ADR 0033 additive-only Open roster synchronization after confirmed Placement plus organization-governed reconciliation when an existing Open assessment is loaded, and population fingerprint authority; Closed membership remains final.
...
- `talent_review_candidate_service.py`: deterministic Review Candidate evaluation/materialization from a Completed Assessment against its exact M3 Review Candidate Policy, plus the two-state (`pending_review`/`reviewed`) Review workflow transition; read-only against Assessment/Result/frozen-population data.
- `talent_official_identification_service.py`: append-only Official Identification decision authority (`identified`/`not_identified`), gated on a Reviewed Review Candidate, exactly one decision per candidate, requiring organization/global access scope in addition to permission; an exact composite FK prevents its duplicated historical context from drifting from the candidate.
- `talent_educator_input_service.py`: bounded qualitative Educator Input authority with historical Placement/Branch resolution (frozen Cycle context or effective-dated Placement at `observed_at`) and append-only amendment/supersession lineage; routers authorize the resolved historical Branch before commit and database uniqueness prevents concurrent lineage forks.
- `talent_learner_profile_service.py`: read-only, per-Student aggregation of authorized M1-M6 historical evidence; it never materializes a profile or changes source records.
- `talent_analytics_service.py` / `talent_analytics_privacy.py`: M9 read-only aggregate query/metric authority and the privacy-decision/complementary-suppression engine (`Cell`/`Group`, `apply_primary_privacy`, `run_complementary_suppression`) shared by every `/api/talent/analytics` route; no production `TalentAnalyticsPrivacyPolicy` is approved yet.
- `routers/`: modular feature routers for users, teachers, subjects, planning, timetable, academic calendar, observations, students, Talent Programs, Assessment Cycles, Student Assessments, Review Candidates, Official Identifications, Educator Inputs, Learner Profiles, Annual Evaluation Plans, and Talent Analytics.
...
- `/api/talent/assessment-cycles`: internal Evaluation-context compatibility/provenance API plus the current-eligibility read used by Student Assessments. ADR 0035 supersedes Draft/Open/frozen-population state as an Assessment eligibility gate. Legacy Open/Close/synchronization endpoints remain for backward compatibility and historical analytics, but normal assessment work does not require them.
- `/api/talent/assessments`: Student Assessment and exact Competency Result API (`routers/talent_assessments.py`, `talent_assessments.view/manage/complete`). Starting an Assessment uses the Student's current effective Academic Placement in the selected Academic Year, Program eligible Grade, authorized Branch scope, and the exact assessable framework/tool already referenced by the Evaluation; Program/Framework lifecycle labels are not additional assessment-start gates. The referenced framework must contain at least one competency and one rubric level; the existing Cycle Population Member row is captured/reused only as historical placement/framework provenance. Results are editable only while the Assessment itself is In Progress and use expected-revision protection; Cycle Draft/Open status is not an editability gate. Completed requires every Framework Competency; it and Incomplete/Insufficient Evidence are read-only. An enabled KPI uses integer weighted contributions over denominator 10,000, nearest-integer ROUND_HALF_UP, and persists Framework-scale result provenance only at completion. Qualitative/no-KPI completion has no result sentinel. Corrections and assessor assignment remain deferred.
...
- `/api/talent/learner-profiles/{student_id}`: read-only longitudinal profile (`routers/talent_learner_profiles.py`, `talent_learner_profiles.view`). Base permission exposes authorized Placement and Assessment history only; Review Candidate, Official Identification, and Educator Input each additionally require their own M6 `.view` permission, otherwise the complete data class and events are omitted. Historical Branch filtering applies independently to every visible record. No profile table, current Talent status, cross-Framework score, audit read event, M8 periods, or Learning Style exists.
- `/api/talent/analytics`: M9 read-only, aggregate-only, privacy-gated analytics API (`routers/talent_analytics.py`, dedicated `talent_analytics.view`/`talent_analytics.view_students`) - `context`, `overview`, `rubric-distribution`, `kpi-distribution`, `competencies`, `breakdowns/{branch|grade|section}`, `period-comparison`, `students`. Every route runs authorized raw aggregate -> `apply_primary_privacy` -> `run_complementary_suppression` -> comparability -> serialization; Candidate/Identification analytics are query-skipped without their own `.view` permission; `kpi-distribution` reports `distribution_mode: unavailable` rather than any numeric bin. `resolve_privacy_policy_provider()` returns `None` in production (fails closed, HTTP 500 `analytics_query_failed`) until a governed privacy policy is approved. No Talent Score/Index/Potential Rate, Talent Map, ranking, export, or AI exists.
...
## Grade-Scoped Talent Rubric Structure And Student History Force Delete

Talent Frameworks support the owner-directed hierarchy `Grade -> Competency -> Rubric Level -> Achievement Description`. `FrameworkCompetency.grade_level` is optional: a Grade value scopes that competency to one eligible Grade, while NULL preserves backward-compatible all-Grade behavior. Grade scope is part of Framework fingerprinting, cloning, Draft-only mutation, tenant scope, and audit semantics. Migration `20260911_002_talent_framework_competency_grade_scope` adds the nullable Grade column.

Achievement wording remains stored at the exact `Framework Competency + Rubric Level + Grade` grain in `talent_grade_competency_rubric_descriptors` (migration `20260911_001_talent_grade_specific_rubric_descriptors`), with the existing generic `Framework Competency + Rubric Level` descriptor retained as fallback for legacy/unscoped Frameworks. Program authoring groups the rubric by Grade, then Competency, then ordered Levels. Student Assessment uses only competencies assigned to the Assessment's recorded historical Grade (plus intentionally unscoped competencies), and resolves the Grade-specific descriptor before generic fallback. Compact downstream result surfaces may show only the ordered rubric number (for example 1-5) with accessible level text and an order-derived low-to-high visual intensity.

Student Assessment Evaluation choices follow the configured Annual Evaluation Plan Period `sequence`. Linked Period sequence is the display-order authority; creation timestamp/id are fallback ordering only for legacy unlinked Cycles.

Student deletion now has a separate high-risk authority, `students.force_delete_history`, exposed through the canonical System Configuration role-permission registry and enabled by default for Administrator only. Normal `students.delete` and `students.bulk_delete` continue to protect Students with Academic Placement or Talent history. A force-delete-capable actor receives a pre-delete history preview and an explicit irreversible confirmation before the Student and Student-owned Academic Placement/Talent history are removed in dependency order. Bulk Delete does not cascade historical data. The force path remains organization/global scoped and backend-enforced; UI confirmation is not authorization.

## Talent Grade-First Rubric Authoring And Re-evaluation Attempts

Per ADR 0036, the owner-facing Talent Program flow is now separated into two responsibilities. Program creation/editing is limited to Program identity plus the Program's eligible Grades for the selected Academic Year. Rubric authoring is a separate Program action. The Rubric workspace renders the Program's eligible Grades as independently collapsible Grade accordions and uses the canonical hierarchy Grade -> Competency -> ordered Rubric Level -> Grade-specific achievement description. Existing Framework/Competency/Rubric/descriptor persistence is reused; no parallel rubric model is introduced.

Completed Student Assessment evidence remains immutable. If a newer saved Framework Version for the same Program has a different semantic fingerprint and contains an assessable Grade-applicable competency/rubric, the current completed Assessment is reported as **Re-evaluation required**. Re-evaluation creates a new Assessment on a new Cycle and links it to the prior Assessment with `reassessment_of_assessment_id`; the prior row is marked `is_current=false`, the replacement remains `is_current=true`, and the existing audit stream records the supersession/reassessment linkage. The historical Assessment's exact Framework, competency results, placement context, Review Candidate, Official Identification, and Educator Input are never rewritten or deleted.

Any Framework Version already referenced by a Student Assessment is semantically immutable even if its lifecycle label is still Draft. Further semantic changes require a new/cloned Framework Version. Current-result analytics read only current Assessment attempts and exclude population members whose Assessment has been superseded, preventing one Student from being double counted after re-evaluation. Migration `20260911_003_talent_assessment_reassessment_attempts` adds the current/reassessment metadata and lookup index.
