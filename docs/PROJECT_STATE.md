...
## Grade-Specific Rubric Descriptors And Administrator History Delete

Talent rubric authoring now supports optional Grade-specific achievement descriptors while preserving the existing generic descriptor as fallback. The additive `talent_grade_competency_rubric_descriptors` table is version/Program/tenant scoped; configuration payloads include Grade identity, Framework semantic fingerprints include the Grade-specific rows, clones copy them, and Student Assessment displays the descriptor for the Assessment's recorded historical Grade. This is intended for rubrics such as Mental Math where the same five proficiency levels have different wording for Grades 1, 2, and 3.
