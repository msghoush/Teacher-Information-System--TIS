...
The Owner-directed Talent evaluation-process rebuild is implemented on `dev`. Program creation/editing is now focused on Program identity plus eligible Grades only. The Programs table exposes a distinct Rubric action. Opening Rubric presents eligible Grades as independent collapsible sections; each Grade owns its Grade-scoped competencies, and each competency displays the shared ordered rubric level scale with Grade-specific achievement descriptions. Saving exits back to a compact Program/Rubric overview rather than continuing the old multi-step onboarding path. The legacy embedded Evaluation Plan surface and backend contracts remain available from the Program overview, but the old Basics -> What we assess -> Evaluation Plan -> Ready wizard is no longer the primary owner-facing Program/Rubric workflow.

ADR 0036 governs the accompanying re-evaluation semantics. `TalentStudentAssessment` now carries additive `is_current` and `reassessment_of_assessment_id` metadata via migration `20260911_003_talent_assessment_reassessment_attempts`. A materially changed newer Framework makes an affected completed current Assessment report **Re-evaluation required**. Starting re-evaluation creates a new Cycle/Assessment attempt against the newer Framework, preserves the old completed Assessment and all dependent historical evidence, marks the old attempt non-current, and records the linkage in the existing Assessment audit stream. Framework Versions referenced by any Assessment are semantically immutable; users must clone/create a new version before editing rubric semantics. Current-result analytics project only current attempts and suppress superseded population members from the current analytical population to avoid double counting.

Production Results & Analytics still depends on governed external provider configuration under ADR 0028. The implementation already contains the production provider seams and approved-value validation; GitHub code does not and must not silently invent or default missing Render environment configuration. A deployment without the governed provider environment variables continues to fail closed by design.
