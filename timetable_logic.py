    ...
    try:
        ...
        for row in versions:
            ...
            version_history.append({
                ...
                "validation_state": (
                    "stale" if row.is_stale else
                    ("validated" if row.approved_at or row.published_at else "not_validated")
                ),
        ...
        "version": (
            {
                ...
                "generated_at": selected_version.generated_at.isoformat() if selected_version.generated_at else "",
                "approved_at": selected_version.approved_at.isoformat() if selected_version.approved_at else "",
                ...
                "validation_state": (
                    "stale" if selected_is_stale else
                    ("validated" if selected_version.approved_at or selected_version.published_at else "not_validated")
                ),
                "source_version_number": next((
                    int(source.version_number) for source in versions
                    if selected_version.source_version_id and int(source.id) == int(selected_version.source_version_id)
                ), None),
