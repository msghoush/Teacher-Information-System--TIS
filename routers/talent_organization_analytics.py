...
import models
from talent_organization_analytics_observability import OrganizationAnalyticsObservation
...
# ---------------------------------------------------------------------------
# B11-B safe operational observability helpers.
#
# These wrap existing decision points (context/availability resolution,
# breadth enforcement, privacy closure) PURELY for telemetry - they never
# recompute a provider decision, never change what is returned/raised, and
# never carry protected Talent data. See
# `talent_organization_analytics_observability.py` for the allowlisted
# field set and the fail-safe emission contract.
# ---------------------------------------------------------------------------

_CONTEXT_ERROR_CATEGORY = {
    "authentication_required": "authentication_authorization_rejection",
    "forbidden": "authentication_authorization_rejection",
    "not_found": "validation_error",
    "organization_scope_required": "validation_error",
}

_BREADTH_ERROR_CATEGORY = {"analytics_breadth_unavailable": "breadth_rejection_failure"}


def _route_error_category(code: str) -> str:
    return _BREADTH_ERROR_CATEGORY.get(code, "validation_error")


def _classify_exception(exc: Exception) -> str:
    """Classify an unexpected exception into one safe, bounded category.

    Uses only the exception's type - never its message/args - so no SQL
    text, bound parameter, or provider-internal detail can leak into
    telemetry.
    """
    ...
    if isinstance(exc, TypeError):
        return "serialization_refusal"
    if exc.__class__.__name__ == "PrivacyClosureError":
        return "privacy_policy_unavailable_failure"
    return "unexpected_analytical_failure"


def _enforce_breadth_observed(observation, breadth_policy, **shape):
    """Observe `svc.enforce_breadth` without recomputing or altering it.

    Reuses the exact shape values the route already computed for the real
    `.allows(...)` call; re-raises every exception unchanged.
    """
    observation.record(
        program_count=shape.get("program_count"), row_count=shape.get("row_count"),
        column_count=shape.get("column_count"),
        prospective_cell_count=shape.get("prospective_cells"),
        relationship_estimate=shape.get("relationship_estimate"),
    )
    if shape.get("prospective_pair_count") is not None:
        observation.record(prospective_pair_count=shape["prospective_pair_count"])
    policy_missing = breadth_policy is None
    try:
        version = svc.enforce_breadth(breadth_policy, **shape)
    except svc.OrganizationAnalyticsError:
        observation.record(breadth_outcome="policy_missing" if policy_missing else "rejected")
        raise
    except Exception:
        observation.record(breadth_outcome="policy_exception")
        raise
    observation.record(breadth_outcome="allowed")
    return version


def _privacy_evaluated(observation, fn, *args, **kwargs):
    """Observe a privacy-closure-producing call (never its raw result)."""
    try:
        result = fn(*args, **kwargs)
    except Exception:
        observation.record(privacy_outcome="provider_exception")
        raise
    observation.record(privacy_outcome="evaluated")
    return result
...
def organization_overview(
    ...
    observation = OrganizationAnalyticsObservation("overview")
    context, error = _b7_context(db, current_user, academic_year_id, availability_provider, observation)
    if error:
        return error
    if policy is None:
    ...
    try:
        ...
        closed = _privacy_evaluated(
            observation, lambda: svc.PrivacyClosedProjectionSet(apply_primary_privacy_and_close(graph, cells, policy)),
        )
        ...
        response = _safe_overview_payload(
...
def organization_talent_map(
    ...
    observation = OrganizationAnalyticsObservation("talent_map")
    context, error = _b7_context(db, current_user, academic_year_id, availability_provider, observation)
    if error:
        return error
    ...
    if candidate_metric and not context.candidate_projection_allowed or identification_metric and not context.identification_projection_allowed:
        observation.emit("rejected", error_category="authentication_authorization_rejection")
    ...
    try:
        ...
        breadth_version = _enforce_breadth_observed(
            observation, breadth_policy, projection_family=f"program_{canonical_dimension}", row_count=len(programs), column_count=len(columns),
        ...
        result = _privacy_evaluated(
            observation, talent_map.build_talent_map_closed, context=context, dimension=canonical_dimension,
            metric=selected_metric, program_ids=selected, column_values=tuple(item["id"] for item in columns),
        ...
        response = talent_map.serialize_talent_map(
...
def _b7_response(*, db, context, policy, breadth_policy, program_ids, observation, branch=None):
    ...
    breadth_version = _enforce_breadth_observed(
        observation, breadth_policy, projection_family="branch_intelligence" if branch else "program_portfolio",
    ...
    result = _privacy_evaluated(
        observation, b7.build_closed_projection,
    ...
    response = b7.serialize_projection(result, programs=programs, metrics=tuple(metrics), branch=branch, context_payload={
        "academic_year_id": context.academic_year_id,
...
def _b7_context(db, current_user, academic_year_id, availability_provider, observation=None):
    """Resolve access context, with optional B11-B availability telemetry.

    `observation` is purely observational: passing `None` reproduces the
    exact pre-B11-B behavior. Availability provider outcomes are inferred
    from the same exception/exception-code distinction the route already
    uses for its own HTTP status mapping - the provider is invoked exactly
    once, never twice, for either purpose.
    """
    provider_missing = availability_provider is None
    try:
        context = svc.resolve_access_context(
            db, user=current_user, academic_year_id=academic_year_id, availability_provider=availability_provider,
        )
    except svc.OrganizationAnalyticsError as exc:
        ...
        if observation is not None:
            if exc.code == "organization_analytics_unavailable":
                observation.record(availability_outcome="provider_missing" if provider_missing else "unavailable")
                observation.emit("unavailable", error_category="commercial_availability_unavailable")
            else:
                observation.emit("rejected", error_category=_CONTEXT_ERROR_CATEGORY.get(exc.code, "validation_error"))
        return None, _error(exc.code, exc.message, 503 if exc.code == "organization_analytics_unavailable" else status)
    except Exception:
        if observation is not None:
            observation.record(availability_outcome="provider_exception")
            observation.emit("failed", error_category="unexpected_analytical_failure")
        return None, _error("organization_analytics_unavailable", "Organization analytics is unavailable.", 503)
    if observation is not None:
        observation.record(availability_outcome="available")
    return context, None
...
def organization_program_portfolio(
    ...
    observation = OrganizationAnalyticsObservation("program_portfolio")
    context, error = _b7_context(db, current_user, academic_year_id, availability_provider, observation)
...
def organization_branch_intelligence(
    ...
    observation = OrganizationAnalyticsObservation("branch_intelligence")
    context, error = _b7_context(db, current_user, academic_year_id, availability_provider, observation)
    if error: return error
    ...
    if policy is None:
        ...
        return _error("organization_analytics_unavailable", "Organization analytics is unavailable.", 503)
    try:
        return _b7_response(db=db, context=context, policy=policy, breadth_policy=breadth_policy, program_ids=program_ids, branch={"id": int(branch_row.id), "name": branch_row.name}, observation=observation)
    except svc.OrganizationAnalyticsError as exc:
        observation.emit("rejected", error_category=_route_error_category(exc.code))
        return _error(exc.code, exc.message, 413 if exc.code == "analytics_breadth_unavailable" else 400)
    except Exception as exc:
...
def organization_participation_overlap(
    ...
    observation = OrganizationAnalyticsObservation("participation_overlap")
    context, error = _b7_context(db, current_user, academic_year_id, availability_provider, observation)
    if error:
        return error
    if policy is None:
    ...
    try:
        ...
        breadth_version = _enforce_breadth_observed(
            observation, breadth_policy, projection_family="participation_overlap",
        ...
        result = _privacy_evaluated(
            observation, participation_overlap.build_closed_projection,
        ...
        response = participation_overlap.serialize_projection(result, programs=programs, context_payload={
        ...
        observation.emit("success", emitted_pair_count=len(response.get("canonical_pairs", ())))
        return response
    except svc.OrganizationAnalyticsError as exc:
        observation.emit("rejected", error_category=_route_error_category(exc.code))
        return _error(exc.code, exc.message, 413 if exc.code == "analytics_breadth_unavailable" else 400)
...
def organization_program_longitudinal(
    ...
    """
    observation = OrganizationAnalyticsObservation(longitudinal.PROJECTION_FAMILY)
    context, error = _b7_context(db, current_user, academic_year_id, availability_provider, observation)
    if error:
        return error
    try:
    ...
    try:
        ...
        breadth_version = _enforce_breadth_observed(
            observation, breadth_policy, projection_family=longitudinal.PROJECTION_FAMILY,
        ...
        result = _privacy_evaluated(
            observation, longitudinal.build_closed_projection,
        ...
        response = longitudinal.serialize_projection(
        ...
        observation.emit(
            "success", period_count=period_count, component_count=component_count,
            authoritative_cycle_count=len(coverage), comparison_count=len(response.get("comparisons", ())),
        )
        return response
    except svc.OrganizationAnalyticsError as exc:
        observation.emit("rejected", error_category=_route_error_category(exc.code))
...
def organization_student_drill(
    ...
    """
    observation = OrganizationAnalyticsObservation("student_drill")
    context, error = _b7_context(db, current_user, academic_year_id, availability_provider, observation)
    if error:
        return error
    ...
    try:
        ...
        breadth_version = _enforce_breadth_observed(
            observation, breadth_policy, projection_family="student_drill", row_count=limit, column_count=field_count,
        ...
        closed_gate, gate_id = _privacy_evaluated(
            observation, student_drill.build_gate_closure, context=context, eligible_count=eligible_count, policy=policy,
        )
        if not student_drill.gate_visible(closed_gate, gate_id):
            observation.emit(
                "rejected", page_limit=limit, error_category="privacy_policy_unavailable_failure",
            )
        ...
        response = student_drill.serialize_projection(result, context_payload={
            "academic_year_id": academic_year_id,
            "authorization_scope": "organization" if context.all_branches else "authorized_branches",
        ...
        observation.emit(
            "success", page_limit=limit, page_returned_count=len(rows), has_more=has_more,
        )
        return response
    except svc.OrganizationAnalyticsError as exc:
        observation.emit("rejected", error_category=_route_error_category(exc.code))
