...
def students_home(request: Request, db: Session = Depends(get_db), current_user=Depends(get_current_user)):
    ...
    can_switch_branches = organization_scope and auth.has_permission(
        db, user, "students.view_all_branches", school_group_id=group_id
    )
    branch_filter = request.query_params.get("branch_id") or ""
    branch_id = (
        int(branch_filter)
        if can_switch_branches and branch_filter.isdigit() and int(branch_filter) in accessible_branch_ids
        else None
    )
    if not can_switch_branches:
        assigned_branch_id = (
            getattr(user, "scope_branch_id", None)
            or getattr(user, "branch_id", None)
        )
        if assigned_branch_id and int(assigned_branch_id) in accessible_branch_ids:
            branch_id = int(assigned_branch_id)
        elif branches:
