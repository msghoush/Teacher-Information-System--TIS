...
@router.post("/auto-match-suggestions")
def get_auto_match_suggestions(
    ...
    """
    Generate auto-match section assignment suggestions for a new teacher based on qualifications.
    
    This endpoint implements the strict major-to-pool-to-priority rules:
    - Maps teacher major to the correct pool
    ...
    """
    from auto_match import generate_auto_match_suggestions
    ...
    if parsed_max_hours is None:
        parsed_max_hours = 24
    allowed_extra = _is_extra_hours_allowed(extra_hours_allowed)
    parsed_extra_hours_count = _parse_int(extra_hours_count) if allowed_extra else 0
    if parsed_extra_hours_count is None:
        parsed_extra_hours_count = 0
    is_national_section_enabled = _teaches_national_section(teaches_national_section)
    
    parsed_national_section_hours = _parse_int(national_section_hours) if is_national_section_enabled else 0
    if parsed_national_section_hours is None:
        parsed_national_section_hours = 0
    try:
        # Generate suggestions
        result = generate_auto_match_suggestions(
            db=db,
            branch_id=branch_id,
            academic_year_id=academic_year_id,
            qualification_keys=raw_qualification_keys,
            max_hours=parsed_max_hours,
            extra_hours_allowed=allowed_extra,
            extra_hours_count=parsed_extra_hours_count,
            national_section_hours=parsed_national_section_hours,
            teachers_national_section=is_national_section_enabled,
        )
        ...
        import logging
        logging.getLogger("uvicorn.error").exception("Error generating auto-match suggestions")
        return JSONResponse(
            status_code=500,
