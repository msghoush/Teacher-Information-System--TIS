...
def analyze_test_workspace(db: Session, organization) -> dict:
    ...
    _add_count(counts, category="SaaS / commercial testing", table="demo_operation_audits", count=_count(db.query(models.DemoOperationAudit).filter(models.DemoOperationAudit.demo_provisioning_id.in_(demo_provisioning_ids))) if demo_provisioning_ids else 0, disposition="test-reset-only")
    ...
    if school_group_id:
        ...
        _add_count(counts, category="Operational tenant", table="demo_access_policies", count=_count(db.query(models.DemoAccessPolicy).filter(models.DemoAccessPolicy.school_group_id == school_group_id)), disposition="tenant-owned")
